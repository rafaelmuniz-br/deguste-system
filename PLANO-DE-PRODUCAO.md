...
- [x] 2.18 **Nova paleta de cores (laranja/escuro) e barra da sacola sólida**, revisando a 2.17: mesma estrutura/estética de app de delivery, só troca a identidade de cor (preto e branco → fundo escuro `#1c1c1c` com laranja `#ff7a1a` como cor de ação) e o tratamento da barra da sacola (era degradê que desaparecia; agora sólida, com borda superior, sempre fixa embaixo). Só CSS (`cardapio.css`, `index.css`) + o teste que descreve o tema (`contraste.test.ts`); nenhuma regra, tela ou dado mudou. WCAG AA continua garantido por teste (19 pares, tema único); 611 testes, lint, typecheck e build passando. **Revisita a decisão "tema claro fixo" da 2.17 — Rafael ainda precisa revisar no PR.** `👤 Lucas`
